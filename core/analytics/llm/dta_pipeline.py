...
def _find_db() -> Path:
    ...
    container's ephemeral filesystem instead, so every DTA run record
    (dta_runs/dta_results) would vanish on the next scheduled redeploy.
...
def _init_dta_schema(db_path: Optional[Path] = None) -> None:
    ...
    for table, col, typedef in [
        ...
        ("dta_results", "matched_indicators", "TEXT"),
    ]:
...
def run_dta_phase2(
    ...
    for transcript in transcripts:
        ...
        for construct_name, construct_def in constructs_to_run.items():
            ...
            raw_response = response.get("text", "") if not response.get("error") else ""
            ...
            if response["error"]:
                ...
                if parsed:
                    ...
                    # Detect which indicator phrases were matched
                    result["matched_indicators"] = _detect_matched_indicators(
                        text, construct_def["indicators"], instances
                    )
                else:
        ...
        conn.execute(
            """INSERT INTO dta_results
               (run_id, participant_id, construct_group, construct_name,
                ...
                instances_json, raw_prompt, raw_response, matched_indicators,
                model, created_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ...
            (
                ...
                r.get("raw_prompt", ""),
                r.get("raw_response", ""),
                json.dumps(r.get("matched_indicators", [])),
                r.get("model", ""),
                now,
            )
        )
...
def load_dta_results(
    ...
    rows = conn.execute(
        """SELECT participant_id, construct_group, construct_name,
                  ...
                  valence_neutral, instances_json,
                  raw_prompt, raw_response, matched_indicators, model
    ...
    df["matched_indicators"] = df["matched_indicators"].apply(
        lambda x: json.loads(x) if x else []
    )
    return df
...
def _detect_matched_indicators(
    participant_text: str,
    indicators: List[str],
    instances: List[Dict],
) -> List[str]:
    """
    Identify which codebook indicator phrases have semantic matches
    in the participant text or in the extracted instance quotes.

    Returns a list of matched indicator strings.
    Uses simple substring matching on normalised text — fast, no API call.
    Semantic matching is left to the LLM; this is for human audit only.
    """
    text_lower = participant_text.lower()
    quote_text = " ".join(
        inst.get("quote", "") for inst in instances
    ).lower()
    combined = text_lower + " " + quote_text

    matched = []
    for indicator in indicators:
        # Normalise: lowercase, strip punctuation for matching
        ind_norm = indicator.lower().replace("'", "").replace('"', "").strip()
        # Check key words (3+ chars) from indicator appear in text
        key_words = [w for w in ind_norm.split() if len(w) >= 4]
        if key_words and all(w in combined for w in key_words[:3]):
